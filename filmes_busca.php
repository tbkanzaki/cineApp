...
<body>
  <div class="wrapper">
    ...
    <div id="content">
      ...
      $opcao = $_GET["opcao"]; //opção da busca - select

      $palavra = $_GET["q"]; //input do search
      $name = "%$palavra%";
      ...
      switch ($opcao) {
        case "1":
          $sql_op1 = "WHERE movies.title LIKE ?"; // SQL with parameters
          $sql = $sql_geral . $sql_op1;
          $stmt = $dbconn->prepare($sql); 
          $stmt->bind_param("s", $name); // here we can use only a variable
        break;

        case "2":
          $sql_op2 = "WHERE movies.synopsis LIKE ?";
          $sql = $sql_geral . $sql_op2;
          $stmt = $dbconn->prepare($sql); 
          $stmt->bind_param("s", $name); 
        break;

        case "3":
          $sql_op3 = "WHERE movies.title LIKE ? or movies.synopsis LIKE ?"; 
          $sql = $sql_geral . $sql_op3;  
          $stmt = $dbconn->prepare($sql); 
          $stmt->bind_param("ss", $name, $name);       
          break;

        case "4":
          $id = $_GET["genero_busca"];
          $sql_op4 = "WHERE movies.genre_id = ? ";
          $sql = $sql_geral . $sql_op4;
          $stmt = $dbconn->prepare($sql); 
          $stmt->bind_param("i", $id); 
        break;

        case "5":
          $id= $_GET["diretor_busca"];
          $sql_op5 = "WHERE movies.direction_id = ? ";
          $sql = $sql_geral . $sql_op5;
          $stmt = $dbconn->prepare($sql); 
          $stmt->bind_param("i", $id); 
        break;
      }  
      ...
      $result->fetch_all(MYSQLI_ASSOC); // all rows matched
